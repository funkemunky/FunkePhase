    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (FunkePhase.getInstance().hasPermission(sender, "help")) {
            if (args.length == 0) {
                ...
                if (FunkePhase.getInstance().hasPermission(sender, "toggle") && args[0].equalsIgnoreCase("toggle")) {
                    FunkePhase.getInstance().toggled = !FunkePhase.getInstance().toggled;
                    sender.sendMessage(ChatColor.GRAY + "AntiPhase has been set to: " + (FunkePhase.getInstance().toggled ? ChatColor.GREEN : ChatColor.RED) + FunkePhase.getInstance().toggled);
                    return true;
                }
                if (FunkePhase.getInstance().hasPermission(sender, "reload") && args[0].equalsIgnoreCase("reload")) {
                    FunkePhase.getInstance().reloadPhase();
                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&aReloaded FunkePhase!"));
                    return true;
                }
                if (FunkePhase.getInstance().hasPermission(sender, "alerts") && args[0].equalsIgnoreCase("alerts") && sender instanceof Player) {
                    ...
                    sender.sendMessage(ChatColor.GRAY + "Toggled alerts: " + (!FunkePhase.getInstance().hasAlertsOn.contains(player.getUniqueId()) ? ChatColor.RED + "false" : ChatColor.GREEN + "true"));
                    return true;
                }
